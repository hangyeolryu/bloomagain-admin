...
/**
 * POST /api/backend/batch-register
 *
 * Proxies to FastAPI POST /api/v1/user/batch-register.
 * Accepts { users: Array<{ userId, username, email? }> } (max 100).
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const upstream = await fetch(
      `${backendUrl.replace(/\/$/, '')}/api/v1/user/batch-register`,
      {
        method: 'POST',
        headers: {

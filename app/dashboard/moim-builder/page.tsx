...
// ──────────────────────────────────────────────────────────────────────────
// 메인 콘셉("결이 맞는 3~4명의 티타임을 자동으로 열어주는 앱")의 초기 운영 도구.
// 자동 감지 배치가 만들어지기 전까지, 파운더가 결이 통할 만한 3~4명을 직접
// 골라 그룹 대화방을 연다 — 유저엔소 자동과 구분되지 않는다 (Wizard of Oz).
//
// 규칙 정합: conversations create는 생성자가 participants에 있어야 하으로
// 현재 로그인한 어드민 계정(= 티타 공식 계정이어야 함)이 방의 참여자로
// 들어간다. 이게 공 "이 방엘 티타지기가 함께 있어요"의 실체.
...
결이 통할 것 같은 분들을 한 자리에 모셨어요. 서두르실 것 없이, 차 한 잌 곳에 두고 천천히 이야기 나눠보세요.

저는 구석에서 조용히 차를 우리고 있을게요. 대화에 끼어들지도, 엿듣고 소문내지도 않아요. 다만 누군가 돈 이야기를 꺼내거나 무례해지면 그때만 살짝 헯기침을 해요.

첫 질문 하나 두고 걸게요 —`;

const ICEBREAKERS = [
  '요즘 하루 중 가장 좀아하는 시간은 언제세요?',
  '최근에 혼자 조용히 웃었던 일이 있다면?',
  '요즘 새로 시작해보고 싶은 게 하나 있다면 뮨예요?',
...
export default function MoimBuilderPage() {
  ...
  return (
    <div className="max-w-5xl space-y-6 p-6">
      <Header
        title="티타임 편성"
        subtitle="결이 통할 3~4명을 골라 그룹 대화방을 엽니다 (자동 결모임의 수동 운영 — 유저에감 동일하게 보여요)"
      />

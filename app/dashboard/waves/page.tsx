                ...
                {waves.map((w) => (
                  <tr key={w.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="hidden md:table-cell px-4 py-2.5">
                      {w.conversationId ? (
                        <Link
                          href={`/dashboard/conversations/view?id=${w.conversationId}`}
                          className="text-xs font-semibold text-green-600 hover:text-green-700 hover:underline whitespace-nowrap"
                        >
                          대화 보기 →
                        </Link>
                      ) : (

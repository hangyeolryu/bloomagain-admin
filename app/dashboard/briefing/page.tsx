...
const NAV = [
  ...
  { id: 'match-qa', label: '⭐ 결 매칭 정량화 Q&A' },
...
const PATENT_QA: { q: string; tag: string; a: React.ReactNode; ask?: string }[] = [
  ...
  },
  {
    ...
    tag: 'PCT·비용',
    a: (
      <>초기창업패키지 선정 시 IP 예산 <b>15–20%</b>를 특허에 배정할 계획입니다. 글로벌은 <b>미국(재외동포 최다)</b>
...
// 투자사가 물은 "결이 맞는다는 걸 어떻게 판단·정량화하나"에 3분 안에 설득되는 답.
// 핵심 3단어: 벡터 · 폐루프 · 축적. 상세 문서: korea repo
// docs/finance/투자QA_결매칭_정량화_2026_07.md
const MATCHING_QA: { q: string; tag: string; a: React.ReactNode }[] = [
  {
    q: '"결이 맞는다"를 어떻게 정량화하나요? (30초 답 — 외워두기)',
    tag: '핵심',
    a: (
      <>결은 형용사가 아니라 <b>벡터</b>입니다. 매일 한 질문(결큐)에 대한 선택이 쌓여 만들어지는
        성향 벡터 + 명시적 관심사 태그 집합 — 두 사람의 적합도는 <b>S = α·코사인(임베딩) +
        β·자카드(태그)</b>로 <b>모든 쌍이 0~1 점수 하나</b>로 계산됩니다. 데이터가 적은 신규
        유저는 α를 0.8로 올려 임베딩 위주로, 답이 쌓이면 0.6/0.4로 <b>자동 전환(적응형)</b>됩니다.</>
    ),
  },
  {
    q: '그 벡터는 어디서 오나요? 프로필 설문이면 다른 앱과 같지 않나요?',
    tag: '데이터 원천',
    a: (
      <>정적 프로필("좋아한다고 <b>말하는</b> 것")이 아니라 <b>매일의 선택("무엇을 고르는가")</b>이
        원천입니다. 결큐 답 하나하나가 태그와 임베딩에 반영돼 <b>답할수록 벡터가 정교해지는
        축적형</b> — 가입날 한 번 쓰는 자기소개와 달리 시간이 지날수록 격차가 벌어집니다.</>
    ),
  },
  {
    q: '그 점수가 진짜 "맞음"을 예측하는지 어떻게 검증하나요? (질문의 진짜 의도)',
    tag: '검증 루프',
    a: (
      <>점수를 내는 데서 멈추지 않고 <b>모든 모임 제안에 예측 점수(minPair)를 기록</b>합니다.
        그 제안이 수락됐는지 → 방이 됐는지 → <b>7일 뒤에도 대화가 살아있는지</b>가 전부
        측정되고, 예측과 결과의 상관으로 임계값·가중치를 조정하는 <b>캘리브레이션 폐루프</b>가
        설계에 들어가 있습니다. 어드민 대시보드에 이미 제안별 minPair가 찍힙니다(시연 가능).
        측정 사다리: 수락률 → 방 생성률 → <b>7일 생존율(북극성)</b> → 대화 왕복수.</>
    ),
  },
  {
    q: '3~4인 모임은 어떻게 조립하나요? 쌍 점수를 그룹으로?',
    tag: '그룹 조립',
    a: (
      <>평균이 아니라 <b>최저 쌍 점수를 최대화(greedy maximin)</b>합니다 — "셋은 친한데 한 명만
        겉도는 방"을 구조적으로 배제. 여기에 <b>상호 top-K</b>(일방적 방 방지), 절대 바닥선,
        28일 쿨다운. 조건(인증·동성만·지역·시간대)은 하드 필터 층으로 <b>점수와 분리</b> —
        가중치 튜닝 지옥 없이 각 층을 독립 검증합니다. 이미 프로덕션 배포 완료.</>
    ),
  },
  {
    q: '유저가 적은데 임베딩·상관분석이 의미 있나요? (콜드스타트)',
    tag: '정직 카드',
    a: (
      <>솔직히 지금은 표본이 작습니다 — 그래서 (1) 신규 구간용 <b>적응형 가중치</b>, (2) 지표를
        <b>먼저 설계해 첫 제안부터 수집</b>, (3) 콜드스타트에도 작동하는 하드 필터 + 상호 top-K.
        저희가 파는 건 "이미 완벽한 알고리즘"이 아니라 <b>유저가 쌓일수록 정확해지는 구조</b>입니다.
        약점을 먼저 꺼내면 신뢰가 됩니다.</>
    ),
  },
  {
    ...
    tag: '해자',
    a: (
      <>수식은 하루면 베끼지만 <b>데이터는 시간으로만 쌓입니다.</b> 45+ 여성의 일상 선택 코퍼스가
        해자이고, 답할수록 정교해지는 구조라 <b>오래 쓸수록 이적 비용이 커집니다.</b> 여기에 안전
        4계층 특허(위험 신호가 매칭 그래프에 되먹임되는 폐루프)가 결합돼 매칭과 안전이 같은
        데이터 흐름에서 돌아갑니다. 정확도 몇 %냐고 물으면 — 지금 숫자를 말하면 거짓말이라
        답하고, <b>측정 파이프라인이 이미 돌고 있음</b>을 보여주는 게 정답입니다.</>
    ),
  },
];
...
export default function BriefingPage() {
  return (
    <div>
      <Header
        title="사업계획·IP 브리핑"
        subtitle="초기창업패키지 재도전 준비 — 2026 회차 미선정, 주관기관 IP 멘토링(7/8, 김한솔 팀장 · 인텔렉추얼디스커버리) 수혜 기록"
        action={
          <button
      ...
      <div className="space-y-4">
        ...
        <SectionCard id="ip" title="2. ⭐ IP 전략 — 특허 (이 자리의 중심)" accent="text-green-700">
          ...
          <div className="rounded-xl border border-gray-200 p-4">
            ...
            <p>
              ...
              정부지원금 IP 예산과의 정합</b>을 자문(선정 시 사업비 15–20% 특허 배정 계획).
            </p>
          </div>
        ...
        {/* 결 매칭 정량화 Q&A — 투자사 질문 대비 */}
        <SectionCard id="match-qa" title="2-C. ⭐ 결 매칭 정량화 Q&A (투자사 대비)" accent="text-green-700">
          <p className="text-gray-600">
            투자사 질문: <b>"결이 맞는다는 걸 어떻게 판단하고 정량화하나요?"</b> — 수식이 아니라
            <b> 검증 가능성</b>을 묻는 질문. 핵심 3단어 <b>벡터 · 폐루프 · 축적</b>으로 답하고,
            어드민의 <b>결모임 자리표 대시보드(제안별 minPair)</b>를 바로 시연하면 강력함.
          </p>
          <div className="space-y-3">
            {MATCHING_QA.map((item, i) => (
              <div key={i} className="rounded-xl border border-gray-200 p-4">
                <div className="mb-1.5 flex items-start gap-2">
                  <Pill tone="green">{item.tag}</Pill>
                  <p className="font-semibold text-gray-900">Q{i + 1}. {item.q}</p>
                </div>
                <p className="text-gray-700">{item.a}</p>
              </div>
          ...
          <div className="rounded-xl bg-gray-50 p-3 text-gray-700">
            <b>화이트보드 한 줄:</b> S_match = α·cos(임베딩) + β·Jaccard(태그), 적응형 α/β →
            그룹은 greedy maximin(최저 쌍 최대화) → 제안마다 minPair 기록 → 수락률·방 생성률·
            <b>7일 생존율</b>과 상관 분석으로 캘리브레이션. 상세: korea repo
            docs/finance/투자QA_결매칭_정량화_2026_07.md
          </div>
        </SectionCard>
        ...
        <SectionCard id="checklist" title="7. 오늘 밤 준비물">
          <ul className="ml-1 space-y-1.5">
            {[
              '초기창업패키지 지원 사업계획서 최신본 열어두기 (멘토가 섹션별로 볼 것)',
